...
float field_of_view_factor = 750;
    ...
    switch (event.type) {
        ...
        case SDL_KEYDOWN: {
            if (event.key.keysym.sym == SDLK_ESCAPE) {
                ...
                // Pressing "1" displays the wireframe and a small red dot for
                ...
                // Pressing “4” displays both filled triangles and wireframe
                // lines
...
vec2_t project(vec3_t point) {
    vec2_t projected_point = {.x = (field_of_view_factor * point.x) / point.z,
                              .y = (field_of_view_factor * point.y) / point.z};

    return projected_point;
};

void update(void) {
    // while (!SDL_TICKS_PASSED(SDL_GetTicks(), previous_frame_time +
    // FRAME_TARGET_TIME));
    int time_to_wait =
    ...
    mat4_t rotation_matrix_X = mat4_create_rotation_x(mesh.rotation.x);

    mat4_t rotation_matrix_Y = mat4_create_rotation_y(mesh.rotation.y);

    mat4_t rotation_matrix_Z = mat4_create_rotation_z(mesh.rotation.z);
    ...
    for (int i = 0; i < num_faces; i++) {
        ...
        for (int j = 0; j < 3; j++) {
            ...
            transformed_vertex =
                mat4_mult_vec4(world_matrix, transformed_vertex);
        ...
        if (dot_normal_camera >= 0) {
            ...
            for (int j = 0; j < 3; j++) {
                vec2_t projected_point =
                    project(vec4_to_vec3(transformed_vertices[j]));
...
void render() {
    ...
    for (int i = 0; i < num_triangles; i++) {
        ...
        if (triangle_ops & 1) {
            draw_filled_triangle(triangle.point[0].x, triangle.point[0].y,
                                 triangle.point[1].x, triangle.point[1].y,
                                 triangle.point[2].x, triangle.point[2].y,
                                 0xFFFFFFFF);
        }

        if (triangle_ops & 1 << 1) {
            draw_triangle(triangle.point[0].x, triangle.point[0].y,
                          triangle.point[1].x, triangle.point[1].y,
                          triangle.point[2].x, triangle.point[2].y, 0xFF00FF00);
        }

        if (triangle_ops & 1 << 2) {
            draw_triangle_vertices(triangle.point[0].x, triangle.point[0].y,
                                   triangle.point[1].x, triangle.point[1].y,
                                   triangle.point[2].x, triangle.point[2].y,
                                   0xFFFF0000);
        }
